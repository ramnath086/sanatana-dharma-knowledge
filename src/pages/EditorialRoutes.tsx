...
function EditorialEntityPage({ entity, preview }: { entity: (typeof editorialEntities)[number]; preview: boolean }) { const [state, setState] = useState<WorkflowState>((entity.workflowState as WorkflowState | undefined) ?? (entity.status === 'published' ? 'PUBLISHED' : 'DRAFT')); const currentReadiness = readiness[entity.id]; const entityClaims = claims.filter((claim) => claim.entityId === entity.id); const entityEvidence = evidence.filter((item) => item.entityId === entity.id); const nextStates = (['DISCOVERED', 'RESEARCHING', 'DRAFT', 'SOURCE_CHECK', 'EDITORIAL_REVIEW', 'TRADITIONAL_REVIEW', 'APPROVED', 'PUBLISHED', 'ARCHIVED'] as WorkflowState[]).filter((next) => canTransition(state, next)); return <div className="editorial-workspace"><div className="preview-banner">{preview ? 'UNPUBLISHED / EDITORIAL PREVIEW' : 'INTERNAL EDITORIAL WORKSPACE'}</div><div className="entity-meta"><span>{entity.type}</span><span>{state}</span><span>{entity.verification}</span></div><p className="entity-description">{entity.description}</p><section className="entity-sections"><section><h2>Publication readiness</h2><p>{currentReadiness.publishable ? 'READY' : 'BLOCKED'} · {currentReadiness.blockers.length} blocker(s)</p>{currentReadiness.blockers.map((item) => <p key={item.code}>{item.message}</p>)}</section><section><h2>Claims & evidence</h2><p>{entityClaims.length} claim(s) · {entityEvidence.length} evidence record(s)</p></section><section><h2>Workflow</h2><div className="filter-list">{nextStates.map((next) => <button type="button" disabled={next === 'PUBLISHED' && !currentReadiness.publishable} onClick={() => setState(next)} key={next}>Move to {next}</button>)}</div></section><section><h2>Relationships</h2>{relationships.filter((relationship) => relationship.from === entity.id || relationship.to === entity.id).map((relationship) => <p key={relationship.id}>{relationship.relationship}: {relationship.from === entity.id ? relationship.to : relationship.from}</p>)}</section></section></div> }
